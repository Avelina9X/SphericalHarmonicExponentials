...
#include "DiffusePrefilterIBL.hpp"

#include "Utils/ReadData.hpp"

void DiffusePrefilterIBL::CreateResources( ID3D12Device *inDevice, D3D_ROOT_SIGNATURE_VERSION inVersion )
{
	{
	...
	{
		std::vector csBlob = DX::ReadData( L"./IBL_DiffusePrefilterCS.cso" );
...
void DiffusePrefilterIBL::Execute( ID3D12GraphicsCommandList *inCommandList, EnvironmentResources &inResources )
{
	...
	if ( inResources.mEnvironmentDataLoaded ) {
		CD3DX12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition( inResources.mDiffuseCubemap.Get(), D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_UNORDERED_ACCESS );
	...
	UINT groupCount[2] = { inResources.kDiffuseCubemapResolution, inResources.kDiffuseCubemapResolution };
	...
	inCommandList->SetComputeRootDescriptorTable( 2, inResources.mDiffuseCubemapUavHandleGPU );

	for ( UINT i = 0; i < 6; ++i ) {
		inCommandList->SetComputeRoot32BitConstant( 0, i % 6, 2 );
		inCommandList->Dispatch( groupCount[0], groupCount[1], 1);
	}

	CD3DX12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition( inResources.mDiffuseCubemap.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COMMON );
	inCommandList->ResourceBarrier( 1, &barrier );
}