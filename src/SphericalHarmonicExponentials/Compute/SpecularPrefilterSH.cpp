...
void SpecularPrefilterSH::Execute( ID3D12GraphicsCommandList7 *inCommandList, EnvironmentResources &inResources )
{
	...
	// Execute final sum accumulation
	{
		...
		CD3DX12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition( mSpecularAccumulator.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE );
		inCommandList->ResourceBarrier( 1, &barrier );
	}
