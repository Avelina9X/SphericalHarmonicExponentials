...
struct SHCoeffsL2Half16
{
    row_major half4x3 c[2];
    float3 bias;
    float pad;
};

struct SHCoeffsL2L4Half16
{
    row_major half4x3 p[6];
    row_major half4x3 q[2];
    float3 bias;
    float pad;
};
...
float3 ReconstructSHL2FromSB( SHBasisL2Half16 shBasis, StructuredBuffer<SHCoeffsL2Half16> buffer, uint index )
{
    half3 acc = mul( shBasis.c_1_4, buffer[index].c[0] );
    acc += mul( shBasis.c_5_8, buffer[index].c[1] );

    return shBasis.c_0 * buffer[index].bias + acc;
}
...
float3 ReconstructSHL2L4FromSB( SHBasisL4Half16 shBasisP, SHBasisL2Half16 shBasisQ, StructuredBuffer<SHCoeffsL2L4Half16> buffer, uint index )
{    
    half3 acc = mul( half4( shBasisP.c_1_4 ), buffer[index].p[0] );
    acc += mul( half4( shBasisP.c_5_8 ), buffer[index].p[1] );
    acc += mul( half4( shBasisP.c_9_12 ), buffer[index].p[2] );
    acc += mul( half4( shBasisP.c_13_16 ), buffer[index].p[3] );
    acc += mul( half4( shBasisP.c_17_20 ), buffer[index].p[4] );
    acc += mul( half4( shBasisP.c_21_24 ), buffer[index].p[5] );
    
    acc += mul( half4( shBasisQ.c_1_4 ), buffer[index].q[0] );
    acc += mul( half4( shBasisQ.c_5_8 ), buffer[index].q[1] );

    return exp( shBasisP.c_0 * buffer[index].bias + acc );
}

Texture2D<float2> gIntegratedBRDF : register( t0 );
StructuredBuffer<SHCoeffsL2Half16> gDiffuseSH : register( t1 );
StructuredBuffer<SHCoeffsL2L4Half16> gSpecularSH : register( t2 );
